...
struct PatientTests{
    ...
    @Test func checkDateofBirth() async throws{
        #expect(throws: MyError.invalidInput("Date of birth cannot be in the future.")){
            try Patient(
                firstName: "Alice",
                lastName: "A",
                height: 162,
                weight: 60,
                medications: [],
                dateOfBirth: Date().addingTimeInterval(60 * 60 * 24) // this adding time interval generated by AI
            )
        }
        let patient = try Patient(
            firstName: "Alice",
